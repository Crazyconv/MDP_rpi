// Test whether disconnection causes reading or writing error.
...
int main(){
	...
	/* When disconect, will dump out off while loop */
	while (read(fd_frcomm, buf, sizeof(buf)) >= 0){
		printf("receive mesage from client: %s\n", buf);
		bzero(buf,sizeof(buf));
	}

	/* When disconect, will print message */
	if(write(fd_frcomm, "Hello!", 6)<0){
		printf("Disconected.\n");
	}
